...
export function AudioCapture() {
  ...
  // ── load devices on mount; also re-sync recording state ─────────────────────
  useEffect(() => {
    const init = async () => {
      try {
        ...
        // Auto-select BlackHole if present, else first device
        const devs: Device[] = devData.devices ?? [];
        // Primary: prefer BlackHole for system audio
        const bh = devs.find((d) => d.is_blackhole);
        const def = devs.find((d) => d.is_default);
        setDeviceIndex(bh?.index ?? def?.index ?? devs[0]?.index ?? null);
        // Mic: prefer built-in MacBook mic, then default non-BlackHole, then any non-BlackHole
        const isBuiltIn = (d: Device) =>
          /macbook|built.?in|internal/i.test(d.name) && !d.is_blackhole;
        ...
        // Reconnect to an in-progress capture
      ...
      <div style={styles.page}>
        <div style={styles.card}>
          ...
          <p style={{ ...styles.muted, marginTop: 16 }}>
            ...
            <strong style={{ color: "var(--text)" }}>BlackHole 2ch</strong>:
            <br />
    ...
    <div style={styles.page}>
      <div style={styles.card}>
        ...
        <p style={styles.muted}>
          Records directly from any audio input device — use{" "}
          <strong style={{ color: "var(--text)" }}>BlackHole 2ch</strong> to capture
          Mac system output at full quality.
        </p>
        ...
        <div style={styles.field}>
          ...
          {micEnabled && (
            <p style={{ ...styles.muted, fontSize: "0.75rem" }}>
              Mixed into the right channel of the WAV (system audio on left, mic on right) for per-speaker diarization.
            </p>
          )}
        </div>
        ...
        {!devices.some((d) => d.is_blackhole) && sdAvailable && (
          <div style={styles.tipBox}>
            ...
            <span style={styles.muted}>
              ...
              BlackHole will appear here as an input device and mirror everything playing on your Mac.
            </span>
          </div>
        )}
      </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  savedBox: {
    ...
    border:       "1px solid rgba(19,239,147,0.2)",
    borderRadius: "var(--radius-sm)",
    padding:      "12px 16px",
    fontSize:     "0.85rem",
    color:        "var(--text)",
